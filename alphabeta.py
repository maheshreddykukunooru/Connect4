...
def isGameOver(board,depth,score):
    ...
    # for i in range(n):
    # 	for j in range(m-3):
    # 		if board[i][j]==board[i][j+1] and board[i][j+1]==board[i][j+2] and board[i][j+2]==board[i][j+3]:  ## Same row
    # 			return True
    # for i in range(n-3):
    # 	for j in range(m):
    # 		if board[i][j]==board[i+1][j] and board[i+1][j]==board[i+2][j] and board[i+2][j]==board[i+3][j]:  ## Same column
    # 			return True
    # for i in range(n-3):
    # 	for j in range(m-3):
    # 		if board[i][j]==board[i+1][j+1] and board[i+1][j+1]==board[i+2][j+2] and board[i+2][j+2]==board[i+3][j+3]:  ## Right diagonal
    # 			return True

    # for i in range(3,n):
    # 	for j in range(0,m-3):
    # 		if board[i][j]==board[i-1][j+1] and board[i-1][j+1]==board[i-2][j+2] and board[i-2][j+2]==board[i-3][j+3]:  ## Left diagonal
    # 			return True
